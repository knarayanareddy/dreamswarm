...
impl Tool for DreamTriggerTool {
    ...
    fn command_signature(&self, _: &Value) -> String { "dream:trigger".into() }
    fn describe_call(&self, input: &Value) -> String {
        let dry = input.get("dry_run").and_then(|v| v.as_bool()).unwrap_or(false);
        if dry { "Dry-run memory consolidation".into() } else { "Run memory consolidation (autoDream)".into() }
    }
    async fn execute(&self, input: &Value) -> anyhow::Result<ToolOutput> {
        let lookback = input.get("lookback_days").and_then(|v| v.as_u64()).unwrap_or(7) as u32;
        let config = DreamConfig { lookback_days: lookback, ..Default::default() };
        let engine = DreamEngine::new(config, std::path::PathBuf::from(&self.working_dir), self.daemon_state_dir.clone());
        let memory = self.memory.read().await;
        let report = engine.dream(&memory, &self.query_engine).await?;
        Ok(ToolOutput { content: DreamReporter::format(&report), is_error: false })
    }
}
