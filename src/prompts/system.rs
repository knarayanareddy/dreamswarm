...
impl SystemPromptBuilder {
    pub fn build(config: &AppConfig, role: Option<&SwarmRole>) -> String {
        let mut sections: Vec<String> = Vec::new();

        sections.push(Self::identity_section(role));
    ...
    fn identity_section(role: Option<&SwarmRole>) -> String {
        let role_fragment = role
            .map(|r| r.system_prompt_fragment())
            .unwrap_or("You are an Autonomous Swarm Worker. Follow instructions precisely, use your tools efficiently, and report findings clearly to the lead agent.");
