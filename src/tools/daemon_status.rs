...
impl Tool for DaemonStatusTool {
    ...
    fn command_signature(&self, _: &Value) -> String { "daemon:status".into() }
    fn describe_call(&self, _: &Value) -> String { "Check KAIROS daemon status".into() }
    async fn execute(&self, input: &Value) -> anyhow::Result<ToolOutput> {
        let include_log = input.get("include_log").and_then(|v| v.as_bool()).unwrap_or(true);
        let log_count = input.get("log_count").and_then(|v| v.as_u64()).unwrap_or(10) as usize;
        let process = DaemonProcess::new(self.state_dir.clone());
        let status = process.status().await?;
        let mut output = format!("## KAIROS Daemon Status\n\nRunning: {}\n", if status.running { "Yes" } else { "No" });
        if let Some(started) = status.started_at {
            output.push_str(&format!("Started: {}\n", started.format("%Y-%m-%d %H:%M UTC")));
        }
        output.push_str(&format!("Actions today: {}\nTokens today: {}\nCost today: ${:.4}\n", status.actions_taken, status.tokens_used_today, status.cost_today_usd));
        if include_log {
            ...
            if !recent.is_empty() {
                ...
                for entry in recent {
                    output.push_str(&format!("[{}] {:?}: {}\n", entry.timestamp.format("%H:%M:%S"), entry.kind, &entry.content[..entry.content.len().min(120)]));
                }
            }
        }
