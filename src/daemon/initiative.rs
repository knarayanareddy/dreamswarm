...
impl InitiativeEngine {
    ...
    pub async fn evaluate(
        ...
        if signals.is_empty() { return Initiative::Sleep; }
        if self.tokens_used_today >= self.config.daily_token_budget || self.cost_today >= self.config.daily_cost_budget {
            return Initiative::Observe("Budget exceeded".to_string());
        }
        if self.trust.is_paused() {
            return Initiative::Observe(format!("Trust too low ({:.0}%)", self.trust.current_level * 100.0));
        }
    ...
    fn rule_based_evaluation(&self, signals: &[Signal]) -> Option<Initiative> {
        let build_failures: Vec<&Signal> = signals.iter().filter(|s| {
            matches!(s.kind, SignalKind::BuildError | SignalKind::TestFailure | SignalKind::CIFailed)
        }).collect();

        if !build_failures.is_empty() {
            if self.trust.should_auto_act(&Urgency::High) {
                return Some(Initiative::Act(ProactiveAction::RunTests {
                    reason: format!("Build/test failure: {}", build_failures[0].description),
                    changed_files: build_failures.iter()
                        .filter_map(|s| s.metadata.get("path").and_then(|p| p.as_str()).map(String::from))
                        .collect(),
                }));
            }
        }

        let source_changes: Vec<&Signal> = signals.iter().filter(|s| {
            s.kind == SignalKind::FileChanged && s.metadata.get("path").and_then(|p| p.as_str())
                .map(|p| p.ends_with(".rs") || p.ends_with(".ts") || p.ends_with(".py")).unwrap_or(false)
        }).collect();

        if source_changes.len() >= 3 {
            if self.trust.should_auto_act(&Urgency::Medium) {
                return Some(Initiative::Act(ProactiveAction::RunTests {
                    reason: format!("{} source files changed", source_changes.len()),
                    changed_files: source_changes.iter()
                        .filter_map(|s| s.metadata.get("path").and_then(|p| p.as_str()).map(String::from))
                        .collect(),
                }));
            }
    ...
    async fn llm_evaluation(&self, signals: &[Signal], query_engine: &QueryEngine) -> Option<Initiative> {
        let signals_text: String = signals.iter().map(|s| {
            format!("- [{:?}] {:?}: {}", s.severity, s.kind, s.description)
        }).collect::<Vec<_>>().join("\n");
...
3. SLEEP
            ...
            self.config.daily_token_budget.saturating_sub(self.tokens_used_today),
            self.config.daily_cost_budget - self.cost_today,
            signals_text
        );

        let messages = vec![serde_json::json!({ "role": "user", "content": tick_prompt })];
        match query_engine.complete(
            "You are a focused daemon evaluating proactive action. Be brief.",
            &messages,
            &[]
        ).await {
            Ok(response) => {
                let text = response.content.iter().filter_map(|b| if b.get("type").and_then(|t| t.as_str()) == Some("text") { b.get("text").and_then(|t| t.as_str()) } else { None }).collect::<Vec<_>>().join("");
                let text = text.trim();
                if text.starts_with("ACT:") {
                    Some(Initiative::Act(ProactiveAction::CustomAction { description: text[4..].trim().to_string(), tool_calls: vec![] }))
    ...
    pub fn trust(&self) -> &TrustSystem { &self.trust }
    pub fn trust_mut(&mut self) -> &mut TrustSystem { &mut self.trust }
}
