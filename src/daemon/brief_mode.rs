...
impl BriefFormatter {
    pub fn format_action(action_type: &str, description: &str, result: &str) -> String {
        ...
        format!("[{}] {} {} -> {}", now, Self::action_emoji(action_type), description, Self::truncate(result, 120))
    }
    ...
    pub fn format_status_bar(trust_pct: f64, actions_today: u64, cost_today: f64, idle_mins: u64) -> String {
        let trust_bar = Self::progress_bar(trust_pct, 10);
        format!("🌙 KAIROS | Trust: {} {:.0}% | Actions: {} | Cost: ${:.3} | Idle: {}m", trust_bar, trust_pct * 100.0, actions_today, cost_today, idle_mins)
    }

    pub fn format_approval_request(action: &str, reasoning: &str) -> String {
        format!("🌙 KAIROS wants to act:\nAction: {}\nReason: {}\n[y/N] > ", Self::truncate(action, 100), Self::truncate(reasoning, 200))
    }
