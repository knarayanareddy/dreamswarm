...
impl MemoryPruner {
    pub fn new(working_dir: PathBuf, confidence_threshold: f64) -> Self {
        Self { working_dir, _confidence_threshold: confidence_threshold }
    }
    ...
    fn is_derivable_content(&self, content: &str) -> bool {
        let indicators = ["fn ", "pub fn ", "struct ", "enum ", "impl ", "trait ", "class ", "def ", "let ", "const "];
        let lines: Vec<&str> = content.lines().filter(|l| {
            let t = l.trim();
            !t.is_empty() && !t.starts_with('#') && !t.starts_with("---")
        }).collect();
        if lines.is_empty() { return false; }
        let code_lines = lines.iter().filter(|l| indicators.iter().any(|ind| l.contains(ind))).count();
        ...
        for line in content.lines() {
            if line.to_lowercase().contains("source:") {
                ...
                if parts.len() >= 2 {
                    ...
                    if !path.is_empty() && path.contains('/') { return Some(path.to_string()); }
                }
            }
        }
        None
