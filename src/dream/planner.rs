...
impl DreamPlanner {
    pub fn plan(mut operations: Vec<MemoryOperation>, config: &DreamConfig) -> Vec<MemoryOperation> {
        Self::deduplicate(&mut operations);
        operations.sort_by_key(|op| Self::priority(&op.kind));
        let validated: Vec<_> = operations.into_iter()
            .filter(|op| Self::validate(op))
    ...
    fn deduplicate(operations: &mut Vec<MemoryOperation>) {
        ...
        operations.retain(|op| {
            let key = format!("{}/{}/{:?}", op.topic, op.subtopic, std::mem::discriminant(&op.kind));
            seen.insert(key)
        });
    }

    fn validate(op: &MemoryOperation) -> bool {
        if op.topic.is_empty() || op.subtopic.is_empty() { return false; }
        match &op.kind {
            OperationKind::Create | OperationKind::Update { .. } | OperationKind::Merge { .. } | OperationKind::Confirm { .. } => {
                !op.content.is_empty() && op.content.len() <= 2000
            }
