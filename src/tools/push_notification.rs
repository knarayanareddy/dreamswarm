...
impl Tool for PushNotificationTool {
    ...
    fn command_signature(&self, _: &Value) -> String { "notify".into() }
    ...
    async fn execute(&self, input: &Value) -> anyhow::Result<ToolOutput> {
        let title = input.get("title").and_then(|v| v.as_str()).unwrap_or("DreamSwarm");
        let message = input.get("message").and_then(|v| v.as_str()).ok_or_else(|| anyhow::anyhow!("Missing message"))?;
        
        #[cfg(target_os = "macos")]
        {
            let _ = tokio::process::Command::new("osascript").args(["-e", &format!("display notification \"{}\" with title \"{}\"", message.replace('"', "\\\""), title.replace('"', "\\\""))]).output().await;
        }
        #[cfg(target_os = "linux")]
        {
            let _ = tokio::process::Command::new("notify-send").args([title, message]).output().await;
        }
        
        Ok(ToolOutput { content: format!("Notification sent: {} - {}", title, message), is_error: false })
    }
}
