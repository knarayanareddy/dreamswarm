...
impl DreamEngine {
    pub fn new(config: DreamConfig, working_dir: PathBuf, daemon_state_dir: PathBuf) -> Self {
        Self { config, working_dir, daemon_state_dir }
    }
    ...
    pub async fn dream(
        ...
        let collector = ObservationCollector::new(self.config.clone(), self.daemon_state_dir.clone());
        ...
        let pruner = MemoryPruner::new(self.working_dir.clone(), self.config.prune_confidence_threshold);
        let prune_ops = match pruner.analyze(memory) {
            Ok(ops) => ops,
            Err(e) => { errors.push(format!("Pruner error: {}", e)); vec![] }
        };
        ...
        let llm_ops = match tokio::time::timeout(
            ...
            Ok(Ok(ops)) => ops,
            Ok(Err(e)) => { errors.push(format!("Analyzer error: {}", e)); vec![] }
            Err(_) => { errors.push("Analyzer timeout".to_string()); vec![] }
        };
        ...
        for op in &planned {
            match self.apply_operation(op, memory) {
                    ...
                    if applied {
                        ...
                        match &op.kind {
                            ...
                            OperationKind::Prune { reason } => {
                                pruned_count += 1;
                                if *reason == PruneReason::Contradicted { contradictions_count += 1; }
                            }
                ...
                Err(e) => {
                    errors.push(format!("Failed to apply {:?} on {}/{}: {}", op.kind, op.topic, op.subtopic, e));
                    if errors.len() > 10 {
        ...
        let report = DreamReport {
            started_at, completed_at, duration_secs: (completed_at - started_at).num_seconds() as u64,
            observations_collected: observations.len(), operations_planned: planned_count, operations_applied: applied_count,
            entries_merged: merged_count, entries_created: created_count, entries_pruned: pruned_count,
            entries_confirmed: confirmed_count, contradictions_resolved: contradictions_count, tokens_consumed: tokens_used,
            cost_usd: cost_used, memory_before_hash, memory_after_hash, errors,
        };

        let daily_log = DailyLog::new(&self.daemon_state_dir)?;
        daily_log.append(&LogEntry {
            timestamp: Utc::now(), kind: LogEntryKind::Dream, content: DreamReporter::format_brief(&report),
            tools_used: vec!["MemoryWrite".into()], tokens_consumed: tokens_used, cost_usd: cost_used,
            trust_level: 1.0, signals_present: vec![],
        })?;
    ...
    fn apply_operation(&self, op: &MemoryOperation, memory: &MemorySystem) -> anyhow::Result<bool> {
        match &op.kind {
            OperationKind::Create => {
                let conf = if op.confidence >= 0.8 { Confidence::Verified } else if op.confidence >= 0.5 { Confidence::Observed } else { Confidence::Inferred };
                let res = memory.writer.store(&op.topic, &op.subtopic, &op.content, Some("autoDream"), conf)?;
                Ok(res.stored)
            }
            OperationKind::Update { existing_path } => {
                memory.topics.append(existing_path, &format!("[autoDream update] {}", op.content), Some("autoDream consolidation"), Confidence::Observed)?;
                memory.index.upsert_pointer(&op.topic, existing_path, &op.content[..op.content.len().min(120)])?;
                Ok(true)
            }
            OperationKind::Merge { source_entries } => {
                memory.writer.store(&op.topic, &op.subtopic, &format!("[Merged by autoDream] {}", op.content), Some("autoDream merge"), Confidence::Verified)?;
            ...
            OperationKind::Prune { reason } => {
                let path = format!("{}/{}.md", op.topic.to_lowercase().replace(' ', "-"), op.subtopic.to_lowercase().replace(' ', "-"));
                let removed = memory.topics.delete(&path).unwrap_or(false);
                memory.index.remove_pointer(&path).ok();
                if removed { tracing::debug!("Pruned {}/{} (reason: {:?})", op.topic, op.subtopic, reason); }
                Ok(removed)
            }
            OperationKind::Confirm { to_confidence, .. } => {
                let conf = if to_confidence == "verified" { Confidence::Verified } else { Confidence::Observed };
                let path = format!("{}/{}.md", op.topic.to_lowercase().replace(' ', "-"), op.subtopic.to_lowercase().replace(' ', "-"));
                memory.topics.append(&path, &format!("[autoDream confirmed] {}", op.content), Some("autoDream confirmation"), conf)?;
                Ok(true)
            }
        }
    }
    ...
    fn snapshot_memory(&self, memory: &MemorySystem) -> anyhow::Result<MemorySnapshot> {
        ...
        let topic_files = memory.topics.list_all()?.iter()
            .filter_map(|path| memory.topics.read(path).ok().flatten().map(|c| (path.clone(), c)))
            .collect();
        Ok(MemorySnapshot { index_content, topic_files })
    }

    fn rollback_memory(&self, memory: &MemorySystem, snapshot: &MemorySnapshot) -> anyhow::Result<()> {
        tracing::warn!("Rolling back memory!");
        std::fs::write(memory.memory_dir().join("MEMORY.md"), &snapshot.index_content)?;
    ...
    fn empty_report(&self, started_at: chrono::DateTime<Utc>, hash: &str) -> DreamReport {
        DreamReport {
            started_at, completed_at: Utc::now(), duration_secs: 0, observations_collected: 0, operations_planned: 0, operations_applied: 0,
            entries_merged: 0, entries_created: 0, entries_pruned: 0, entries_confirmed: 0, contradictions_resolved: 0, tokens_consumed: 0,
            cost_usd: 0.0, memory_before_hash: hash.to_string(), memory_after_hash: hash.to_string(), errors: vec![],
        }
    }
}
