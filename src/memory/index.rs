...
impl MemoryIndex {
    ...
    pub fn load_raw(&self) -> anyhow::Result<String> {
        if !self.path.exists() { 
            return Ok(String::from("# DreamSwarm Memory Index\n\n_No memories stored yet._\n"));
        }
        ...
        for line in content.lines() { 
            ...
            if trimmed.starts_with("→ ") || trimmed.starts_with("-> ") {
                ...
                if let Some(dash_pos) = rest.find(" — ") {
                    ...
                    let subtopic = file_path.split('/').last().unwrap_or(&file_path).trim_end_matches(".md").to_string();
                    entries.push(IndexEntry {
                        topic: current_topic.clone(),
                        subtopic, file_path, description,
                    });
                    ...
                    let subtopic = file_path.split('/').last().unwrap_or(&file_path).trim_end_matches(".md").to_string();
                    entries.push(IndexEntry {
                        topic: current_topic.clone(), subtopic,
    ...
    pub fn upsert_pointer(&self, topic: &str, file_path: &str, description: &str) -> anyhow::Result<()> {
        ...
        if self.path.exists() {
            ...
            for line in content.lines() {
                ...
                if trimmed.starts_with("## ") {
                    ...
                    if !is_same_file {
                        topics.entry(current_topic.clone()).or_default().push(trimmed.to_string());
                    }
                } 
            }
        }
    ...
    pub fn list_topics(&self) -> anyhow::Result<Vec<String>> { 
        ...
        let mut topics: Vec<String> = entries.iter().map(|e| e.topic.clone()).collect::<std::collections::HashSet<_>>().into_iter().collect();
        topics.sort();
        Ok(topics) 
    }
    ...
    pub fn find_relevant(&self, query: &str) -> anyhow::Result<Vec<IndexEntry>> { 
        ...
        let mut scored: Vec<(IndexEntry, usize)> = entries.into_iter().filter_map(|entry| {
            let haystack = format!("{} {} {}", entry.topic, entry.subtopic, entry.description).to_lowercase();
            let score: usize = query_words.iter().filter(|word| haystack.contains(*word)).count();
